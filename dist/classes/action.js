...
class KaminoAction {
    ...
    async addDepositIxV2() {
        const { collateralFarmAccounts: farmsAccounts } = await KaminoAction.getFarmAccountsForReserve(await this.getObligationPda(), this.reserve, this.kaminoMarket.farmsProgramId);
    ...
    async addDepositObligationCollateralIxV2() {
        ...
        const { collateralFarmAccounts: farmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.reserve, this.kaminoMarket.farmsProgramId);
    ...
    async addBorrowIxV2() {
        ...
        const obligationAddress = await this.getObligationPda();
        const { debtFarmAccounts: farmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.reserve, this.kaminoMarket.farmsProgramId);
    ...
    async addRepayAndWithdrawV2Ixs(withdrawCollateralAmount) {
        ...
        const { collateralFarmAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.outflowReserve, this.kaminoMarket.farmsProgramId);
    ...
    async addDepositAndWithdrawV2Ixs(withdrawCollateralAmount) {
        ...
        const { collateralFarmAccounts: depositFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.reserve, this.kaminoMarket.farmsProgramId);
        const { collateralFarmAccounts: withdrawFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.outflowReserve, this.kaminoMarket.farmsProgramId);
    ...
    async addDepositAndBorrowIxV2() {
        const obligationAddress = await this.getObligationPda();
        const { collateralFarmAccounts: collateralFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.reserve, this.kaminoMarket.farmsProgramId);
        this.lendingIxsLabels.push(`depositReserveLiquidityAndObligationCollateralV2`);
        this.lendingIxsLabels.push(`borrowObligationLiquidityV2`);
        this.lendingIxs.push((0, instructions_1.depositReserveLiquidityAndObligationCollateralV2)({
            liquidityAmount: this.amount,
        ...
        const { debtFarmAccounts: debtFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.outflowReserve, this.kaminoMarket.farmsProgramId);
        let borrowIx = (0, instructions_1.borrowObligationLiquidityV2)({
            liquidityAmount: this.outflowAmount,
        }, {
            borrowAccounts: {
    ...
    async addRepayAndWithdrawIxsV2(withdrawCollateralAmount) {
        ...
        const { debtFarmAccounts: debtFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.reserve, this.kaminoMarket.farmsProgramId);
        let repayIx = (0, instructions_1.repayObligationLiquidityV2)({
            liquidityAmount: this.amount,
        }, {
            repayAccounts: {
        ...
        const { collateralFarmAccounts: collateralFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.outflowReserve, this.kaminoMarket.farmsProgramId);
        this.lendingIxs.push((0, instructions_1.withdrawObligationCollateralAndRedeemReserveCollateralV2)({
            collateralAmount: withdrawCollateralAmount,
        }, {
            withdrawAccounts: {
    ...
    async addLiquidateIxV2(maxAllowedLtvOverridePercent = 0) {
        ...
        const obligationAddress = await this.getObligationPda();
        const { collateralFarmAccounts: collateralFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(obligationAddress, this.outflowReserve, this.kaminoMarket.farmsProgramId);
        const { debtFarmAccounts: debtFarmsAccounts } = await KaminoAction.getFarmAccountsForReserve(await this.getObligationPda(), this.reserve, this.kaminoMarket.farmsProgramId);
        ...
        for (const kaminoReserve of reserves) {
            if (mode === types_1.ReserveFarmKind.Collateral && kaminoReserve.state.farmCollateral !== utils_1.DEFAULT_PUBLIC_KEY) {
                farms.push([
                    ...
                    await (0, utils_1.obligationFarmStatePda)(kaminoReserve.state.farmCollateral, obligationAddress, this.kaminoMarket.farmsProgramId),
                    kaminoReserve,
                ]);
            }
            if (mode === types_1.ReserveFarmKind.Debt && kaminoReserve.state.farmDebt !== utils_1.DEFAULT_PUBLIC_KEY) {
                farms.push([
                    ...
                    await (0, utils_1.obligationFarmStatePda)(kaminoReserve.state.farmDebt, obligationAddress, this.kaminoMarket.farmsProgramId),
                    kaminoReserve,
                ]);
            }
        }
    ...
    async addInitObligationForFarm(reserve, mode, addAsSupportIx = 'setup') {
        ...
        if (mode === types_1.ReserveFarmKind.Collateral && (0, utils_1.isNotNullPubkey)(reserve.state.farmCollateral)) {
            const pda = await (0, utils_1.obligationFarmStatePda)(reserve.state.farmCollateral, obligationAddress, this.kaminoMarket.farmsProgramId);
        ...
        if (mode === types_1.ReserveFarmKind.Debt && (0, utils_1.isNotNullPubkey)(reserve.state.farmDebt)) {
            const pda = await (0, utils_1.obligationFarmStatePda)(reserve.state.farmDebt, obligationAddress, this.kaminoMarket.farmsProgramId);
            const account = await (0, kit_1.fetchEncodedAccount)(this.kaminoMarket.getRpc(), pda);
            if (!account.exists) {
    ...
    static async getFarmAccountsForReserve(obligationAddress, reserve, farmsProgramId) {
        ...
        if ((0, kit_1.isSome)(collateralFarmAddress)) {
            collateralFarmAccounts = {
                obligationFarmUserState: (0, kit_1.some)(await (0, utils_1.obligationFarmStatePda)(collateralFarmAddress.value, obligationAddress, farmsProgramId)),
        ...
        if ((0, kit_1.isSome)(debtFarmAddress)) {
            debtFarmAccounts = {
                obligationFarmUserState: (0, kit_1.some)(await (0, utils_1.obligationFarmStatePda)(debtFarmAddress.value, obligationAddress, farmsProgramId)),
